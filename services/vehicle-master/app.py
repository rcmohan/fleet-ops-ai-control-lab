...
def create_app() -> FastAPI:
    ...
    def list_vehicles(
        ...
        items = sorted(store.list(), key=lambda item: item["vehicleId"])
        if fleet_id:
    ...
    def create_vehicle(
        ...
        return store.create(
            "veh", "vehicleId", data, "syntheticVin", idempotency_key
        )
    ...
    def assign_fleet(
        ...
        return {
            key: record[key]
            for key in ("vehicleId", "fleetId", "sourceVersion")
        }
    ...
    def assign_unit(
        ...
        record = store.mutate(
            ...
            lambda value: value.update(
                telematicsUnitId=body.telematics_unit_id
            ),
        )
        return {
            key: record[key]
